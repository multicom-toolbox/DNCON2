...
use constant{
	...
	FILTALN  => abs_path(dirname($0)).'/P3_filter_dncon_aln_mmseq2.pl',
...
confess 'Oops!! alnstat scripts not found!'  if not -f ALNSTAT;
...
foreach my $c (sort {$COVERAGE{$a} <=> $COVERAGE{$b}} keys %COVERAGE){
	...
	if ((count_lines("$hhbid.aln") > $T) && (count_lines("$hhbid.aln") > 2000)){
		...
		if(count_lines("$hhbid.aln") > 1){
				$ren_return_val=system("perl ".FILTALN." $hhbid.aln $id.aln");
				while($ren_return_val){
					$count = $count+1;
					$ren_return_val=system("perl ".FILTALN." $hhbid.aln $id.aln");
					if($count == 20){
						 $count = 0;
    ...
    system_cmd("cp $jhmid.aln ${id}_orig.aln");
	   ...
	   open COLSTATS, "<$jhmid.colstats" or confess $!;
		while(<COLSTATS>){
			$jhm_n=<COLSTATS>;
			$jhm_neff =<COLSTATS>;
			print "jhm_n:$jhm_n jhm_neff:$jhm_neff\n";
			last;
		}
		last;
	}
...
if ($jackaln_neff>$jhm_neff){
	if (count_lines("$hhbid.aln") < count_lines("$id.jackaln")){
			...
			#system_cmd(FILTALN." -id 90 -i $hhbid.a3m -o $id.a3m");
			#system_cmd("egrep -v \"^>\" $id.a3m | sed 's/[a-z]//g' > $id.aln");
			if(count_lines("$hhbid.aln") > 1){
				$ren_return_val=system("perl ".FILTALN." $hhbid.aln $id.aln");
				while($ren_return_val){
					$count = $count+1;
	  ...
	  # system_cmd(FILTALN." -id 90 -i $jhmid.a3m -o $id.a3m");
		# system_cmd("egrep -v \"^>\" $id.a3m | sed 's/[a-z]//g' > $id.aln");
		if(count_lines("$jhmid.aln") > 1){
			#system_cmd("perl ".FILTALN." $jhmid.aln $id.aln");
			$ren_return_val=system("perl ".FILTALN." $jhmid.aln $id.aln");
			while($ren_return_val){
				$count = $count+1;
				$ren_return_val=system("perl ".FILTALN." $jhmid.aln $id.aln");
				if($count == 20){
